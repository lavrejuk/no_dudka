...
if(tg){tg.ready();tg.expand();const s=document.documentElement.style;
s.setProperty('--bg',tg.backgroundColor||'#fff');s.setProperty('--text',tg.textColor||'#000');
s.setProperty('--button',tg.buttonColor||'#3390ec');s.setProperty('--button-text',tg.buttonTextColor||'#fff');
s.setProperty('--hint',tg.hintColor||'#999');s.setProperty('--secondary-bg',tg.secondaryBackgroundColor||'#f0f0f0');}
...
const defaultEmojis=['🚬','💨','🍺','🍷','🎮','📱','🍔','💊','🎯','🏃','📚','💤'];
function showScreen(n){[scW,scS,scT,scSet].forEach(s=>s.classList.remove('active'));
({welcome:scW,setup:scS,tracker:scT,settings:scSet})[n].classList.add('active');
if(tg?.BackButton){(n==='setup'||n==='settings')?tg.BackButton.show():tg.BackButton.hide()}}
...
function resolveUserName(){userName=tg?.initDataUnsafe?.user?.first_name||tg?.initDataUnsafe?.user?.username||'Друг';
welcomeTitle.textContent='Привет, '+userName+'!'}
function renderEmojiPicker(sel='🎯'){selectedEmoji=sel;emojiPicker.innerHTML='';
defaultEmojis.forEach(e=>{const sp=document.createElement('span');sp.className='emoji-option'+(e===sel?' selected':'');
sp.textContent=e;sp.addEventListener('click',()=>renderEmojiPicker(e));emojiPicker.appendChild(sp)})}
function setDefaultDateTime(){const n=new Date();n.setMinutes(n.getMinutes()-n.getTimezoneOffset());lastUseDatetime.value=n.toISOString().slice(0,16)}
function fillDateTime(ts){const d=new Date(ts);d.setMinutes(d.getMinutes()-d.getTimezoneOffset());lastUseDatetime.value=d.toISOString().slice(0,16)}
function clearSetupFields(){setupNameInput.value='';dailyCostInput.value='0';cigPerDayInput.value='';cigPackCostInput.value='';vapeMonthlyCostInput.value='';
...
function selectHabit(type){fromSettings=false;editingHabitId=null;
...
setupSaveBtn.textContent='➕ Добавить привычку';setupBackBtn.onclick=goToWelcome;showScreen('setup');setDefaultDateTime()}
function openSetupFromSettings(){fromSettings=true;editingHabitId=null;currentSetupHabit=null;
...
setupSaveBtn.textContent='➕ Добавить привычку';setupBackBtn.onclick=()=>{showScreen('settings');renderSettingsList()};showScreen('setup');setDefaultDateTime()}
function openEditHabit(h){fromSettings=true;editingHabitId=h.id;currentSetupHabit={...h};
...
setupSaveBtn.textContent='💾 Сохранить';setupBackBtn.onclick=()=>{showScreen('settings');renderSettingsList()};showScreen('setup')}
function showCustomInput(){customBlock.style.display='block';customInput.focus()}
function confirmCustom(){const v=customInput.value.trim();if(!v)return alert('Введи название привычки');
...
setupSaveBtn.textContent='➕ Добавить привычку';setupBackBtn.onclick=goToWelcome;showScreen('setup');setDefaultDateTime()}
function saveHabitFromSetup(){const dt=lastUseDatetime.value;if(!dt)return alert('Укажи дату и время');
let name,icon,costPerDay=0;
if(editingHabitId){name=setupNameInput.value.trim();if(!name)return alert('Введи название');icon=selectedEmoji||'🎯';costPerDay=parseFloat(dailyCostInput.value)||0}
else if(currentSetupHabit?.type==='cigarettes'){name='Сигареты';icon='🚬';costPerDay=((parseFloat(cigPerDayInput.value)||0)/20)*(parseFloat(cigPackCostInput.value)||0)}
else if(currentSetupHabit?.type==='vape'){name='Вейп';icon='💨';costPerDay=(parseFloat(vapeMonthlyCostInput.value)||0)/30}
else{name=setupNameInput.value.trim();if(!name)return alert('Введи название привычки');icon=selectedEmoji||'🎯';costPerDay=parseFloat(dailyCostInput.value)||0}
...
if(editingHabitId){const i=habits.findIndex(h=>h.id===editingHabitId);if(i!==-1)habits[i]=hd}else{habits.push(hd)}
saveHabits();goToTracker()}
function resetHabit(id){pendingResetId=id;confirmModal.style.display='flex'}
confirmYes.addEventListener('click',()=>{if(pendingResetId!==null){const h=habits.find(x=>x.id===pendingResetId);
if(h){h.startTime=Date.now();saveHabits();renderHabits();updateAllTimers()}pendingResetId=null}confirmModal.style.display='none'});
confirmNo.addEventListener('click',()=>{pendingResetId=null;confirmModal.style.display='none'});
function deleteHabit(id){if(!confirm('Точно удалить эту привычку?'))return;
...
function formatTimeDiff(ms){const t=Math.floor(ms/1000);
return`${Math.floor(t/86400)}д ${Math.floor((t%86400)/3600)}ч ${Math.floor((t%3600)/60)}м ${t%60}с`}
function getAchievement(d){if(d>=365)return'🏆 Год без!';if(d>=180)return'🎉 Полгода!';if(d>=90)return'💪 3 месяца!';
if(d>=30)return'🌟 Месяц!';if(d>=7)return'🔥 Неделя!';if(d>=1)return'👍 1 день!';return''}
function renderHabits(){if(habits.length===0){habitsContainer.innerHTML='<p style="margin-top:20px;">У тебя пока нет привычек. Добавь их в настройках!</p>';
totalMoneyEl.textContent='';timerElements=[];return}
habitsContainer.innerHTML='';timerElements=[];
habits.forEach(h=>{const diff=Math.max(0,Date.now()-h.startTime),days=Math.floor(diff/86400000);
...
rb.addEventListener('click',e=>{e.stopPropagation();resetHabit(h.id)});card.appendChild(rb);
habitsContainer.appendChild(card);timerElements.push({element:tm,id:h.id})})}
function renderSettingsList(){if(habits.length===0){settingsListContainer.innerHTML='<p style="margin-top:10px;">Список пуст.</p>';return}
settingsListContainer.innerHTML='';
habits.forEach(h=>{const item=document.createElement('div');item.className='settings-list-item';
...
bg.appendChild(eb);bg.appendChild(db);item.appendChild(sp);item.appendChild(bg);settingsListContainer.appendChild(item)})}
function showSettings(){renderSettingsList();showScreen('settings')}
function updateAllTimers(){let total=0;
timerElements.forEach(({element,id})=>{const h=habits.find(x=>x.id==id);
if(h){const diff=Math.max(0,Date.now()-h.startTime);element.textContent=formatTimeDiff(diff);total+=(h.costPerDay||0)*(diff/86400000)}});
...
function factoryReset(){if(!confirm('⚠️ Ты уверен? Все привычки будут удалены.'))return;
...
function init(){resolveUserName();loadHabits();
